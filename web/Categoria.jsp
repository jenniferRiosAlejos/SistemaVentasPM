   ...
   <div class="d-flex justify-content-between p-4">
        <div class="card col-sm-3">
            ...
            <div class="card-body">
                ...
                <form action="Controlador?menu=Categoria" method="POST" onsubmit="return validateForm()">
                    <div class="mb-3">
                        <label class="form-label">Nombre Descripción</label>
                        <input type="text" value="${categoria.getDescrip()}" name="txtDesc" class="form-control" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Estado</label>
                        <select name="txtEstado" class="form-select" required>
                            <option value="">Seleccionar</option>
                            <option value="Activo" ${categoria.getEstado() == 'Activo' ? 'selected' : ''}>Activo</option>
                            <option value="Inactivo" ${categoria.getEstado() == 'Inactivo' ? 'selected' : ''}>Inactivo</option>
                        </select>
                    </div>
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <c:forEach var="cate" items="${categorias}">
                        <tr>
                            <td>${cate.getDescrip()}</td>
                            <td>${cate.getEstado()}</td>
                            <td>
                                <a class="btn btn-warning" href="Controlador?menu=Categoria&accion=Editar&id=${cate.getId()}"><img src="img/edit.png" alt="Editar"/></a>
                                <!-- <a class="btn btn-danger" href="Controlador?menu=Categoria&accion=Eliminar&id=${cate.getId()}"><img src="img/delete.png" alt="Eliminar"/></a> -->
                            </td>
                        </tr>
                    </c:forEach>
                </tbody>
